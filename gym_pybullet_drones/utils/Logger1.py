...
class Logger(object):
    ...
    def plot(self, pwm=False):
        """Plot the xyz position and xyz velocity over time.
        ...
        # 创建 2 行 1 列的子图，分别用于位置和速度
        fig, axs = plt.subplots(2, 1, figsize=(10, 8))  # 2 rows, 1 column for xyz position and xyz velocity
        
        t = np.arange(0, self.timestamps.shape[1] / self.LOGGING_FREQ_HZ, 1 / self.LOGGING_FREQ_HZ)

        #### Plot xyz position ###################################################
        for j in range(self.NUM_DRONES):
            axs[0].plot(t, self.states[j, 0, :], label=f"Drone {j} X")
            axs[0].plot(t, self.states[j, 1, :], label=f"Drone {j} Y")
            axs[0].plot(t, self.states[j, 2, :], label=f"Drone {j} Z")
        ...
        #### Plot xyz velocity ###################################################
        for j in range(self.NUM_DRONES):
            axs[1].plot(t, self.states[j, 3, :], label=f"Drone {j} Vx")
            axs[1].plot(t, self.states[j, 4, :], label=f"Drone {j} Vy")
            axs[1].plot(t, self.states[j, 5, :], label=f"Drone {j} Vz")
