### narrow_gap_motion_primitives_direction_only_upward.py
"""
四机编队窄缝对比实验：固定刚性编队的上绕版本。

与当前 `narrow_gap_motion_primitives_direction_only.py` 的区别：
- 不加入显式纯侧移原语
- 不惩罚正向 z 运动
- 使用更大的刚性碰撞半径，更容易出现上绕行为
"""
...
def generate_directional_path_library(
    ...
    """Generate the earlier direction-only primitive set that tends to climb over obstacles."""
    if arc_lengths is None:
        arc_lengths = [0.6]
    if arc_radii is None:
        arc_radii = [np.inf, 18.0, 12.0, 8.0, 6.0, 4.0, 3.0, 2.0, 1.0, 0.5, 0.3]
...
DEFAULT_RIGID_EXTRA_MARGIN = 10.0 * DEFAULT_DRONE_SIZE
...
def _compute_endpoint_collision_mask(endpoints, kd_tree, formation_safe_radius):
...
def _compute_ground_collision_mask(
    endpoints,
...
def _select_leader_goal(
    ...
    """Earlier greedy endpoint selection that tends to produce upward detours."""
    ...
    obstacle_collision_mask = _compute_endpoint_collision_mask(
        endpoints,
        OBSTACLE_KD_TREE,
        formation_safe_radius,
    )
    ...
    total_costs = np.where(feasible_mask, goal_dists, np.inf)
...
def _rigid_collision_radius(base_offsets):
    """Use a large rigid-formation collision radius to reproduce upward detours."""
...
def _plot_goal_distance(goal_distance_history):
    ...
    plt.title("Direction-only upward experiment: distance to goal")
...
def run(
    ...
    """Run the rigid-formation upward-detour baseline."""
    ...
    for step in range(int(duration_sec * env.CTRL_FREQ)):
        ...
        goal_distance_history.append(
            (step / env.CTRL_FREQ, float(np.linalg.norm(center_goal - leader_pos)))
        )
    ...
    summary_lines = [
        "=== Fixed formation upward summary ===",
    ...
    summary_path = save_summary_text(output_path, "fixed_formation_upward_metrics.txt", summary_lines)
    saved_figure_paths = []
    ...
    if plot:
        safety_fig = plot_safety_histories(
            min_pair_distance_history,
            min_obstacle_clearance_history,
            title="Fixed formation upward: safety histories",
        )
        traj_3d_fig = plot_3d_trajectories(
            ...
            title="Fixed formation upward: 3D trajectories",
        )
        top_view_fig = plot_top_view_trajectories(
            ...
            final_positions=final_positions,
            obstacle_specs=obstacle_specs,
            title="Fixed formation upward: top-view trajectories",
        )
        figure_specs = [
            (safety_fig, output_path / "fixed_formation_upward_safety.png"),
            (traj_3d_fig, output_path / "fixed_formation_upward_3d.png"),
            (top_view_fig, output_path / "fixed_formation_upward_top.png"),
        ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Four-drone narrow-gap comparison: upward rigid-formation direction-only motion primitives"
    )
